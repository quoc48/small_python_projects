...
def main():
    ...
    When I say:     That means:
        ...
        Fermin      One digit is correct and in the right position.
        Bagels      No digit is correct.
    
    For example, if the secret number was 248 and your guess was 843, the clues
    would be Fermin Pico'''.format(NUM_DIGITS))
...
def getSecretNum():
    ...
    # Get the right NUM_DIGITS digits in the list for the secret number:
    secretNum = ''
...
def getClues(guess, secretNum):
    ...
    for i in range(len(guess)):
        if guess[i] == secretNum[i]:
            ...
            clues.append('Pico')
    if len(clues) == 0:
        return 'Bagels' # There are no digits at all.
    else:
